...
class Tool():
    ...
    def draw_heatmap(self, words):
        ...
        plt.savefig("./results/word_heatmap.png")
        # plt.show()
    ...
    def draw_scatter(self, words):
        ...
        plt.savefig("./results/sematic.png")
        # plt.show()


if __name__ == '__main__':
    tool = Tool("./results/result_embed.json")
    near_list = tool.find_near_word('man', 9)
    words = [value for key, value in near_list]
    tool.draw_heatmap(words)
    tool.draw_scatter(words)
    loss_data = np.loadtxt("train_loss.csv", delimiter=',')
    plot_loss(loss_data, "Training loss")
